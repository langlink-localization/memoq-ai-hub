...
`v1.0.16` 主要优化更新检查体验和下拉选项显示：

- 检查更新现在会在超时后明确结束，不再让 Dashboard 长时间停在“Checking / 检查中”，并提供更友好的中英文提示。
- Updates 卡片在检查时会保留上一次已知版本；远端版本与当前版本相同时会显示“已是最新”。
- Select 输入框和下拉菜单会更稳定地占满内容区域，较长的 Provider、模型、资产和路径名称会自然换行。
