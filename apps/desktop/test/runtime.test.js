...
test('runtime writes real translation history using configured provider route', async () => {
  ...
  try {
    ...
    assert.equal(state.historyExplorer.items[0].runtime.desktopVersion, '1.0.2');
...
test('runtime exposes runtime identity in desktop version payload', async () => {
  ...
  try {
    ...
    assert.equal(payload.desktopVersion, '1.0.2');
    assert.equal(payload.runtime.desktopVersion, '1.0.2');
